        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ config('settings.currency_symbol') }} {{number_format($order->discount, 2)}}</td>
                    ...
                    <td>{{config('settings.currency_symbol')}} {{number_format($order->totalPrice() -
                        $order->receivedAmount(), 2)}}</td>
                    <td> <a href="{{url('admin/download/invoice/'.$order->id)}}"><i class="fa-solid fa-file-pdf"
                                style="color: rgb(230, 3, 3); font-size: 25px;"></i></a>
                    </td>
                    <td style="vertical-align: middle">{{
                        \Carbon\Carbon::parse($order->created_at)->translatedFormat('l, j F Y H:i:s') }}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
